...
def CompMove():
    ...
    if not move:
        ...
        time.sleep(1)
        square_group.empty()
        background = p.image.load('Tie.png')
        background = p.transform.scale(background, (WIDTH, HEIGHT))
...
blank_image = p.image.load('black.png')
x_image = p.image.load('x image.png')
o_image = p.image.load('o image.png')
background = p.image.load('xo.png')
